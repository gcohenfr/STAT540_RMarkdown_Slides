...
date: "2020/01/15 <br><br> <font size=5> Slides by: Sara Mostafavi with contributions from Keegan Korthauer </font>"
output:
...
* The *standard error* of $X$ is $\frac{\sigma}{\sqrt{n}}$
...
ggplot(data.frame(x = x_bar), aes(x = x)) +
  geom_density(color = "blue", cex=2) +
  geom_line(stat = "function", fun = dnorm, args = list(mean=0.5, sd=sqrt(0.125/n)), color = "pink", cex=2, linetype="dashed") + xlim(0,1) + ggtitle("Sampling Distribution of mean(X)")
```
]
...
ggplot(data.frame(x = c(0, 1)), aes(x = x)) +
  geom_line(stat = "function", fun = dbeta, 
            args = list(shape1=0.5, shape2=0.5),
            color = "blue", cex=2)+ ggtitle("Distribution of X")
```
]
...
* Given the expression level of gene $\normalsize g$ in some patients with disease (e.g. cancer), and some healthy (control) individuals, determine if gene $\normalsize g$ is differentially expressed in cancer vs healthy samples
