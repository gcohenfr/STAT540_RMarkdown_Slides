...
By default, `lm` assumes a <font color = "red">**reference-treatment effect**</font> parametrization (mathematically, we need *more* dummy variables, see [math handout](math_handout.html))
...
It is *important* to remember that `lm` reports *simple, not main* effects!! <font color="blue"> why?? because of the parametrization used!! </font> (see see [math handout](math_handout.html))
...
In  previous examples, we simplified the comparisons and assume that each factor have 2 levels (e.g., E16 *vs* 4W)
...
```{r,echo=FALSE}
summary(lm(gExp~gType*devStage,hitDat))
```

Note that all these parameters are still *simple* effects, but we now have more (one for each level of `devStage`)
...
``` {r, include = FALSE}
twoFactFit <- lm(formula = gExp ~ gType * devStage, data = plot1Dat)
```
