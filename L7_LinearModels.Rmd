...
Note that we may not be interested in these hypotheses, e.g., $\tau_{KO}$ and $\tau_{4W}$ are *conditional* effects (*at* a given level of a factor)
---
class: middle
...
```
<small> $^*$ Summary of `lm` shown for the gene in the left plot
...
---
class: middle
### Statistically significant interaction effect: non-parallel pattern 
...
plot1Lim <- ggplot(plot1Dat, aes(x = devStage, y = gExp, group=gType, colour=gType)) + 
  ...
  scale_color_manual(values = c('wt' = '#00BFC4', 'NrlKO' = '#F8766D'))+
  geom_segment(aes(x=1,y=mu.hat1[1,2],xend=1,yend=mu.hat1[2,2]),colour=1,arrow = arrow(length = unit(0.1,"cm")))+
  geom_segment(aes(x=2,y=mu.hat1[3,2],xend=2,yend=mu.hat1[4,2]),colour=1,arrow = arrow(length = unit(0.1,"cm"))) 
...
When the interaction effect is significant, the *conditional* effects may not agree: compare the genotype effect @E16 with that @4W!

*Main* effects (overall, regardless of level) may mask interesting patterns! (more discussed later) 
...
Note that the interaction effect is not significant (almost parallel pattern).
...
egFit <- lm(gExp ~ gType * devStage, subset(egDat,gene=="1431651_at"))
summary(egFit)$coeff
```
...
When the interaction is not significant, we may want to test the *overall* association between the response and a factor

> **Note**: a significant interaction means that the effect of a factor depends on the level of the other. Thus, main effects may mask interesting results!
...
We can use the function `anova()` in R to test the effect of a factor, on average over the levels of the other 

> `anova` uses type II sums of squares, which follows the principle of marginality (order of variables in the model matters)

```{r,highlight.output = c(4:5)}
tidy(anova(lm(gExp ~ gType * devStage, subset(egDat,gene=="1431651_at"))))
```

As we suspected in slide #25, there is a significant genotype effect for this gene, i.e., its mean expression changes in NrlKO group (compared to wt), on average over develpmental stages.
---
<big>
