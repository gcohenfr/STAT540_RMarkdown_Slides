...
lucky +
  ...
  geom_text(aes(x = 2.2, y = abs(mu.hat[3,2]+mu.add)/2, label = TeX("$\\widehat{\\tau_{4W}}$", output = "character")),colour='grey', size=3, parse = TRUE) +
  #interaction
geom_segment(aes(x=2.1,y=mu.hat[4,2],xend=2.1,yend=mu.add),colour='blue',arrow = arrow(length = unit(0.1,"cm")))+
  ...
  geom_text(aes(x = 2.2, y = abs(mu.hat[4,2]+mu.add)/2, label = TeX("$\\widehat{\\tau_{KO4W}}$", output = "character")),colour="blue", size=3, parse = TRUE) 
```
...
$\tau_{KO4W}$ | gTypeNrlKO:devStage4_weeks | complicated, see additional slides | *interaction* effect of NrlKO and 4W
