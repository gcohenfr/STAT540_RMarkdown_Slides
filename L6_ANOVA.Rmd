...
.pull-left[
<font color = red> y ~ x: </font> formula, y numeric, x numeric and/or factor
]

.pull-right[
<font color = red> yourData: </font> data.frame in which x and y are to be found (we recommend this style)
]
<br>
...
class: middle
``` {r}
means.dev <- as.data.frame(miniDat %>% group_by(devStage) %>% summarize(cellMeans=mean(gExp)))
means.dev
data.frame(cellMeans=means.dev[1,2],txEffects=diff(means.dev$cellMeans))
```
