...
### - use linear regression to model the association of multiple variables with the response
...
knitr::include_graphics("L6_ANOVA_files/t_aov_lm.png")
```
...
and $\tau_2=\mu_2-\mu_1=E[Y_{i2}] -E[Y_{i1}]$ compares the group means
---
...
class: middle
  ...
  ![](L6_ANOVA_files/more_2_groups.png)
...
$$Y_{ij} = \theta+\tau_2 * x_{ij} + \varepsilon_{ij};\; \; x_{ij}=\bigg\{\begin{array}{l} 
1\text{ if } j=2\\
0 \text{ otherwise}\\
\end{array}$$
...
> Note that using both formulas $Y_{i1} = \theta + \varepsilon_{i1}$

> Note that using both formulas $Y_{i2} = \theta + \tau_2+ \varepsilon_{i2}$
...
### <font color = "red">t-test</font>: a special case of a linear model
...
### <font color = "red">1-way ANOVA with more than 2 levels</font>: another special case 
$$Y_{ij} = \theta+\tau_2 * x_{ij2} + \tau_3 * x_{ij3} +\varepsilon_{ij};\; \; x_{ij2}=\bigg\{\begin{array}{l}
1\text{ if } j=2\\
0 \text{ otherwise}\\
...
---
class: middle
### With a special matrix $X$, we can write our model in matrix notation:
...
---
class: middle
## $X$ with **categorical** variables (cont.)
...
```
<center>
```{r,echo=FALSE, out.height="500px",out.width="850px"}
knitr::include_graphics("L6_ANOVA_files/devStg.png")
```
</center>
---
class: middle
...
.pull-left[
<font size=5>
**Estimate**: $\hat\tau_{P2}=\hat\mu_{P2}-\hat\mu_{E16}$

$H_0: \tau_{P2}=\mu_{P2}-\mu_{E16}$
]
.pull-right[
...
.pull-left[
<font size=5>
**Estimate**: $\hat\tau_{4weeks}=\hat\mu_{4weeks}-\hat\mu_{E16}$

$H_0: \tau_{4weeks}=\mu_{4weeks}-\mu_{E16}$
]
