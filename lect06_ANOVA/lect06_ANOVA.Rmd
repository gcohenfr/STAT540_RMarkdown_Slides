...
```{r, include = FALSE}
knitr::opts_chunk$set(tidy = FALSE, tidy.opts=list(width.cutoff=80))
...
$$Z_1, Z_2, ..., Z_{n_Z}$$
$$\hat{\mu}_Y = \bar{Y} = \frac{\sum_{i=1}^{n_Z} Z_i}{n_Z}$$
...
```{r, eval=TRUE, echo = FALSE, tidy=TRUE, linewidth=50}
...
```
Linear regression model
```{r, eval=TRUE, echo = FALSE, linewidth=50}
summary(lm(gExp ~ gType, data=miniDat, subset = gene == "Irs4"))
```
]]
...
```{r,echo=FALSE, out.width="720", fig.align="center"}
...
```{r, echo=FALSE, fig.width=10.5, fig.height=3.2, dev='svg', fig.align="center"}
...
```
<div style= "top: -50px;">
```{r, echo=FALSE, fig.align="center", out.width=825}
...
``` {r,echo=FALSE,highlight.output = c(3:6), highlight.color="pink"}
means.dev <- as.data.frame(devDat %>% subset(gene=="geneB") %>% group_by(devStage) %>% summarize(cellMeans=mean(gExp)))
means.dev %>% mutate(txEffects=cellMeans-cellMeans[1])
```
...
**Estimate**: $\hat{\tau}_{P2}=\hat{\mu}_{P2}-\hat{\mu}_{E16}=\bar{Y}_{P2}-\bar{Y}_{E16}$
]
.pull-right[
...
```{r,echo=FALSE, out.width="775", fig.align="center"}
